...
public class HookEngine {
    ...
    public enum ActionType {
        ...
        //返回一段提示文本（最轻量，仅做注入）
        PROMPT("prompt"),
        //发送 HTTP 请求（webhook 场景）
        HTTP("http"),
        //启动子 Agent 处理（最重，需要外部注册 runner）
        AGENT("agent");
    ...
    /**
     * 在工具真正执行之前，跑所有为 PRE_TOOL_USE 事件注册的 hook，决定"要不要拦下这次工具调用"。
    ...
    private HookResult executeCommand(Hook h, HookContext ctx) {
        ...
        try {
            ...
            // 注入环境变量
            env.put("DEVECODE_EVENT", ctx.event() != null ? ctx.event().value() : "");
            env.put("DEVECODE_TOOL", ctx.toolName() != null ? ctx.toolName() : "");
            env.put("DEVECODE_FILE_PATH", ctx.filePath() != null ? ctx.filePath() : "");
