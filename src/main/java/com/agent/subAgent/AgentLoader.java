 ...
 *   <li>User-level definitions from {@code ~/.devecode/agents/*.md}</li>
 *   <li>Project-level definitions from {@code <projectRoot>/.devecode/agents/*.md}</li>
 * </ol>
 * project>user>内置,后出现的配置覆盖先出现的配置
 */
public final class AgentLoader {
    //AgentName-SubAgentSpec
    ...
    public static Map<String, SubAgentSpec> loadAll(Path projectRoot) {
        ...
        if (home != null && !home.isEmpty()) {
            loader.loadDir(Path.of(home, ".devecode", "agents"));
        }

        if (projectRoot != null) {
            loader.loadDir(projectRoot.resolve(".devecode").resolve("agents"));
        }
    ...
    /**
     * 解析单个Agent 定义文件,存放在YAML frontmatter中
     * 从YAML frontmatter中加载name、description、tools、disallowedTools、model、maxTurns
     * 正文不空则视为systemPromptOverride
     */
